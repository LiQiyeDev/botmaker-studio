 ...
 * {@code MethodLock} documented {@code GameLoop.run}'s body as the user's while {@code FileRole} locked the file
 * around it, and whichever caller asked last won. The render path asked {@code FileRole} and locked the whole
 * game loop; nothing asked on the write path at all. Both now ask here, so they cannot drift apart again — if
...
public record LockResolver(ProjectConfig config, ProjectTemplate template, Path file) {
    ...
    private String reasonFor(ASTNode node, EditKind kind) {
        ...
        if (lock.locksBody()) {
            return methodName(node) + " is generated by BotMaker — edit the run() method instead.";
        }
    ...
    private static MethodDeclaration enclosingMethod(ASTNode node) {
        ...
        // reachable only from CompilationUnit.getCommentList(). Walking up therefore finds nothing, and a
        // comment inside GameLoop.run would be judged by its file and locked, in the one method the file
        // exists to let the user write. Fall back to the source range that actually contains it.
