...
/**
 * Turns a rendered expression into a drop target. Every place an expression is shown is a slot the user can
 * drag into, whether or not it was built through {@link SentenceLayoutBuilder} — the operands of a binary
 * expression and a declaration's initialiser go straight into an {@code HBox}, and were silently the only
 * expressions in the editor that refused a drop.
 *
