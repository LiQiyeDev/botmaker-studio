 ...
 * {@code before} is simply what the canvas was, and undo puts it back.
 *
 * <p>Snapshots are cheap because the flow is small — a few dozen cards, each a name, two doubles and a flag —
 * and because the card <em>objects</em> are shared, not copied. That sharing is deliberate: the drafts are
 * bound to the side panel's fields, so an undo has to bring back the very object the panel is editing, not an
 * equal one. It is also why {@code S} must have a value-based {@code equals} — a no-op "mutation" (a drag that
 * put the card back where it started) is dropped by comparing before with after, and nothing else can tell.
 *
 * @param <S> the snapshot type: an immutable value describing everything undo restores
 */
public final class FlowHistory<S> {
     ...
     */
    public FlowHistory(Supplier<S> capture, Consumer<S> restore) {
