...
- **2026-09-02 — JDK 25 LTS and JavaFX 25.0.4.** `pom.xml` (`maven.compiler.release` 25 in place of
  ...
  their bases. Note that Studio's suite already fails 78 tests on `main` from the SDK dependency's
  removal; that count is unchanged by this, verified by re-running with the pom stashed.
  Full account of the constellation-wide move in `../botmaker-studio-api/ROADMAP.md`.

  **The `package` matrix broke twice on the way, and neither fault was visible from a green `build` job** —
  it only runs on a tag or a dispatch, so both were found by dispatching it deliberately.

  1. **JDK 25 ships no `jmods` directory**, so `<modulePath>${java.home}/jmods</modulePath>` was a path that
     does not exist and jlink refused before doing anything. JEP 493 (JDK 24) lets jlink link from the
     run-time image instead, and a JDK built with that capability drops `jmods` entirely. The element is
     deleted, not repointed; the module list is untouched. One local consequence is now written into the
     pom: run-time-image linking refuses a JDK whose config a distribution has patched, so `-Pdist` on
     **Fedora's** `java-25-openjdk` fails with `java.security has been modified` — a bare `jlink` by hand
     says the same, so build installers with an upstream JDK.
  2. **`packaging/linux/botmaker-studio.spec` had to be re-forked from JDK 25's template**, exactly as its
     own header had warned since August: 25's template carries a common-scripts placeholder in `%pre` and
     `%preun` that 21's does not, so jpackage 25 had nowhere to put those scripts and `rpmbuild` exited 1.
     The diagnosis cost is worth recording — jpackage swallows rpmbuild's stderr and prints only *"Warning:
     app-image dir not generated by jpackage"*, so the message names neither the spec nor the placeholder;
     the real line (`unknown tag: {`) appears only by rerunning `rpmbuild` by hand against the generated
     spec that `--temp` leaves behind. **And the first re-fork was wrong in a way worth knowing about
     forever: jpackage substitutes placeholders over the whole file, comments included**, so a header
     comment *naming* the placeholder spliced a shell function body into the spec's own header. The file now
     opens with that warning. The one-line delta — the `$1 = 0` guard so an upgrade does not unregister the
     desktop entry — is re-applied and verified in the built package with `rpm -qp --scripts`.
...
- **2026-08-30 — nothing is locked but `main`'s signature.** With nothing generated there is nothing to protect an edit
  ...
  bundled library source, and the *signature* of `public static void main(String[])`. That signature survives
  because it is the one the JVM looks for, and breaking it is the one edit whose consequence the user cannot
  read off the screen; it is matched on the method's shape rather than the file's path, so renaming or moving
  the entry point keeps the rule. `main`'s **body** is the user's — it is where the bot is put together, one
  static API call at a time. Gone with the machinery: the method lock badge and its accent, the
  pinned trailing `return` in an activity's `run()`, the explorer's *Generated by BotMaker* group and its
  hidden derived files (every file is now listed, under **My code** or **Library**), and the locked-parts
  diff `CodeExecutionService` ran before every write to disk.
...
- **2026-08-28 — Project ▸ Manage Plugins… (plugin ecosystem, phase 9).** `ui/app/ManagePluginsDialog` over
  ...
  Libraries uses — so the plugin lands on the project's classpath and `PluginHost.bind` finds it through the
  same `ServiceLoader` pass that finds the SDK.

  **Why so little machinery.** `META-INF/services` already says how a plugin on the classpath is
  instantiated; the only thing missing was the **coordinate**, and a coordinate is all the registry serves.
  A bespoke install path would be a privilege the bundled SDK plugin has and a third party's does not.

  Three rules worth keeping: the version installed is the entry's `verifiedVersion` (what the registry's gate
  actually checked, not the newest tag); *installed* is decided by coordinate and never by version, so a
  plugin pinned older reads as installed and its version stays Manage Libraries' business; and an unreachable
  registry is an empty catalog with a message, never a blocked editor.

  Checked end to end across three repositories: `botmaker publish --dry-run` wrote an entry, the registry's
  `tools/build-index.sh` generated the index from it, and `PluginRegistry.parse` read that file back whole.

- **2026-08-28 — publishing writes one gallery file instead of rewriting the index (plugin ecosystem,
  phase 8b).** `botmaker-gallery` moved to `bots/<owner>-<repo>.json` with `index.json` generated by its own
  CI, so `BotPublisher` creates or updates **one file** and unpublishing **deletes** one. `editIndex`,
  `editIndexRemove`, `mergeEntry` and `removeEntry` are gone with the array they merged.

  **Why.** Appending an entry to a shared array makes every concurrent submission a merge conflict, and each
  publish was a read-modify-write against a base SHA somebody else may have moved. Unpublishing rewrote the
  whole file, so the diff a maintainer had to review was the entire gallery for a one-line removal.

  **Reading is untouched, and deliberately so.** `GitHubGallery.browse()` still fetches `index.json` from the
  same raw URL; the generated array is content-identical, so every Studio already in the wild keeps browsing
  and installing. **Publishing from one of those breaks on purpose** — the gallery's `validate.yml` refuses a
  pull request that edits `index.json`, with a message naming the Studio update. A conversion job was the
  alternative and was declined: it means maintaining both shapes indefinitely.

  `GitHubConfig.entryPath(owner, repo)` is where the identity is decided, `GitHubClient.delete(url, body,
  token)` is new (the Contents API's file deletion needs a body, and `HttpRequest.DELETE()` sends none), and
  `BotPublisherIndexTest` now covers the path and the bytes of one entry file rather than a merge that no
  longer happens.
