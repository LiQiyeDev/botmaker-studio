...
#
# NEVER WRITE A PLACEHOLDER'S NAME IN THESE COMMENTS. jpackage substitutes them by plain text replacement
# over the whole file, comments included — so naming one here splices a shell function body into the header
# and rpmbuild dies with "unknown tag: {" on whatever line the expansion happens to break. That is a real
# hour, spent on 2026-09-02. Refer to them in prose ("the common-scripts placeholder"), never verbatim.
#
# FORKED FROM: JDK 25, jrt:/jdk.jpackage/jdk/jpackage/internal/resources/template.spec. It must stay matched
# to the JDK that runs jpackage — CI pins temurin 25 in both jobs (setup-java in .github/workflows/ci.yml).
# The placeholder set is NOT stable across releases, and this file is the proof: it was forked from JDK 21
# and re-forked on 2026-09-02 when CI moved to 25, because 25's template gained a common-scripts placeholder
# in its %pre and %preun sections that 21's does not have. jpackage 25 had nowhere to put those scripts and
# rpmbuild exited 1 on both runners — with jpackage swallowing rpmbuild's own stderr and reporting only
# "Warning: app-image dir not generated by jpackage", so the symptom names neither the spec nor the missing
# placeholder. **If java-version moves again, re-fork from that JDK's template and re-apply the delta below
# rather than editing this file in place.** To read the template out of a JDK that ships no `jmods`
# directory (JDK 25 does not — see the jlink comment in the pom), read it through the jrt filesystem:
...
# 0 on erase, 1 on upgrade). Reported by the maintainer, 2026-08; still unfixed upstream as of JDK 25.
#
# The "true;" prefix mirrors the idiom upstream already uses in the pre section: it keeps the block
# syntactically valid if the desktop-uninstall placeholder ever expands to nothing (it would, with
